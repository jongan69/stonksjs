...
/**
 * Comprehensive Demo of stonksjs Packages (CommonJS Version)
 *
...
async function demonstrateFinvizAPI() {
  ...
  try {
    ...
    log('red', `❌ Error getting AAPL quote: ${error.message}`);
  }
  ...
  for (const symbol of symbols) {
    try {
      ...
      log('red', `❌ Error getting ${symbol}: ${error.message}`);
    }
  }
  ...
  try {
    ...
    screeners.slice(0, 10).forEach((screener, index) => {
      console.log(`${index + 1}. ${screener.name} (ID: ${screener.id})`);
    });
  } catch (error) {
    log('red', `❌ Error getting screeners list: ${error.message}`);
  }
  ...
  try {
    ...
    topGainers.slice(0, 10).forEach((symbol, index) => {
      console.log(`${index + 1}. ${symbol}`);
    });
  } catch (error) {
    log('red', `❌ Error getting top gainers: ${error.message}`);
  }
}
...
async function demonstrateQuotePackage() {
  ...
  try {
    ...
    log('red', `❌ Error in cache test: ${error.message}`);
  }
}
...
async function demonstrateStockScreener() {
  ...
  try {
    ...
    console.log('\nSample Filters:');
    filters.slice(0, 15).forEach((filter, index) => {
      console.log(`${index + 1}. ${filter.name}`);
    });
  } catch (error) {
    log('red', `❌ Error getting filters: ${error.message}`);
  }
  ...
  for (const { name, method } of performanceScreeners) {
    try {
      ...
      log('red', `❌ Error getting ${name}: ${error.message}`);
    }
  }
...
async function demonstrateAdvancedUsage() {
  ...
  try {
    ...
    log('red', `❌ Error in portfolio analysis: ${error.message}`);
  }
  ...
  try {
    ...
    log('red', `❌ Error in sentiment analysis: ${error.message}`);
  }
  ...
  try {
    ...
    log('red', `❌ Error in custom screener: ${error.message}`);
  }
}
